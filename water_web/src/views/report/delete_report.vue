  ...
  methods: {
    ...
    async selectOptionReg(option){
      ...
      if(option.id == 0){
        ...
        try{
        const res = await fetch(this.$store.state.hostname + '/OquvMarkazPupilGroupDetailInfo/getPaginationUdalitQilinganlarListSearcByNote?page=0&size=100&note=' + option.name);
